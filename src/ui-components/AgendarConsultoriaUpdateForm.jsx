...
export default function AgendarConsultoriaUpdateForm(props) {
  const {
    id: idProp,
    agendarConsultoria: agendarConsultoriaModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = agendarConsultoriaRecord
      ? { ...initialValues, ...agendarConsultoriaRecord }
      : initialValues;
  ...
  const [agendarConsultoriaRecord, setAgendarConsultoriaRecord] =
    React.useState(agendarConsultoriaModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? await DataStore.query(AgendarConsultoria, idProp)
        : agendarConsultoriaModelProp;
      setAgendarConsultoriaRecord(record);
    };
    queryData();
  }, [idProp, agendarConsultoriaModelProp]);
  React.useEffect(resetStateValues, [agendarConsultoriaRecord]);
  const validations = {
    email: [{ type: "Required" }],
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await DataStore.save(
            AgendarConsultoria.copyOf(agendarConsultoriaRecord, (updated) => {
      ...
      {...getOverrideProps(overrides, "AgendarConsultoriaUpdateForm")}
      {...rest}
    >
      <TextField
        label="Email"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || agendarConsultoriaModelProp)}
